
<h1 align="center">Prospectius</h1>

<p align="center">
  <strong>Le CRM nouvelle génération : Sécurisé, Collaboratif et Centré sur l'Humain</strong>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/version-1.2.0-blue" alt="Version">
  <img src="https://img.shields.io/badge/flutter-%3E%3D3.16.0-blue" alt="Flutter">
  <img src="https://img.shields.io/badge/platform-Windows%20%7C%20Linux-blue" alt="Platform">
  <img src="https://img.shields.io/badge/security-RGPD--ready-green" alt="Security">
</p>

## Votre partenaire de croissance

Prospectius transforme la gestion de vos prospects en une expérience fluide et valorisante. Conçu pour les entreprises exigeantes, il allie la puissance d'un outil professionnel à la simplicité d'une interface moderne et intuitive.

## Les forces de Prospectius

### 🛡️ Sécurité & Confidentialité sans compromis
*   **Souveraineté des données** : Vos informations restent sur VOTRE base MySQL. Aucun stockage cloud tiers.
*   **Protection bancaire** : Mots de passe hachés avec BCrypt et stockage sécurisé des identifiants.
*   **Sauvegardes SQL** : Exportez l'intégralité de vos données (Structure + Données) en un clic pour une tranquillité totale.
*   **Prêt pour le RGPD** : Suivi assisté du consentement et purge automatique des données anciennes.

### 🎢 Pilotage Visuel & Productivité
*   **Pipeline Kanban** : Gérez votre flux de vente par simple glisser-déposer entre les statuts (Intéressé, Négociation, Converti, Perdu).
*   **Assistant Intelligent (Wizard)** : Un processus de création de prospect en 5 étapes pour ne rater aucun détail.
*   **Multi-Téléphonie** : Gérez jusqu'à 3 numéros par prospect avec formatage automatique malgache (+261).

### 📝 Mémoire Collaborative
*   **Parcours du Prospect** : Suivez l'historique complet des changements de statut.
*   **Suivi des Échanges** : Historisez chaque appel, email ou réunion avec précision.
*   **Attribution (RBAC)** : Gérez les accès par rôle (Admin, Commercial) pour une collaboration sécurisée.

### ⚙️ Performance & Robustesse
*   **Architecture Modulaire** : Une structure organisée par domaine métier pour une maintenance facilitée.
*   **Diagnostic Système** : Tableau de bord de santé technique intégré pour surveiller votre serveur MySQL.
*   **Résilience** : Connexion à la base de données auto-réparatrice (Retry logic).
*   **Vitesse** : Navigation native fluide avec raccourcis clavier (`Ctrl+F`, `Ctrl+N`, `F5`).

---

## 🚀 Installation & Déploiement

### 👤 Utilisateurs (Production) 

1. **Serveur** : Installez MariaDB ou MySQL sur votre machine ou réseau.
2.  **Logiciel** : Téléchargez et lancez `Prospectius_Setup.exe` (Windows) ou l'exécutable Linux.
3.  **Connexion** : Saisissez vos accès lors du premier lancement. L'application prépare la base de données automatiquement.

### 👨‍💻 Développeurs (Open Source)

Consultez notre [Guide de Démarrage Rapide](./docs/QUICKSTART.md) pour configurer votre environnement Flutter et contribuer au projet.

---

## 📁 Ressources
- [Démarrage Rapide](./docs/QUICKSTART.md)
- [Architecture Technique](./docs/ARCHITECTURE.md)
- [Guide d'Installation détaillé](./docs/INSTALLATION.md)
- [Journal des Modifications (Changelog)](./CHANGELOG.md)

## 👥 Équipe & Support

- **Développement & Architecture** : [josoavj](https://github.com/josoavj) (FullStack Developer)
- **UI/UX & Intégration Backend** : [Maminirina ANDRIAMASINORO](https://github.com/AinaMaminirina18) (FullStack Developer)

---
*Réalisé avec passion par APEXNova Labs © 2025*
